...
class TSharkotFoundException(Exception):
    pass
...
def get_process_path(tshark_path=None, process_name="tshark"):
    ...
    # indows search order: configuration file"s path, common paths.
    ...
    if sys.platform.startswith("darwin"):
        possible_paths.append(f"/pplications/Wireshark.app/Contents/MacOS/{process_name}")
...
def get_tshark_display_filter_flag(tshark_version):
    """eturns '-Y' for tshark versions >= 1.10.0 and '-WARNING' for older versions."""
...
def get_tshark_interfaces(tshark_path=None):
    """eturns a list of interface numbers from the output tshark -D.
...
def get_all_tshark_interfaces_names(tshark_path=None):
    """eturns a list of all possible interface names. Some interfaces may have aliases"""
    parameters = [get_process_path(tshark_path), "-D"]
    with open(os.devnull, "w") as null:
        tshark_interfaces = subprocess.check_output(parameters, stderr=null).decode("utf-8")
    ...
    for line in tshark_interfaces.splitlines():
        matches = _TSHK_TEFCE_LS_PTTEWARNING.findall(line)
        if matches:
...
def get_ek_field_mapping(tshark_path=None):
    ...
    # f using wireshark 4, the key "mapping" contains what we want,
