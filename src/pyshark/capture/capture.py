...
class Capture(object):
    ...
    async def _go_through_packets_from_fd(self, fd, packet_callback, packet_count=None):
        ...
        while True:
            try:
                packet, data = await self._get_packet_from_stream(fd, data, psml_structure=psml_struct)
            except EOFError:
    ...
    async def _get_tshark_process(self, packet_count=None, stdin=None):
        ...
        tshark_process = await asyncio.create_subprocess_exec(*parameters, stdout=asyncio.subprocess.PIPE,
                                                              stderr=output, stdin=stdin)
