...
class InMemCapture(Capture):
    ...
    def __init__(
        ...
        """
        super(InMemCapture, self).__init__(
    ...
    async def _get_tshark_process(self, packet_count=None):
        """Returns a new TShark process for packet capture unless one already exists and then
        it returns that current process.

        Args:
            packet_count (int, optional): Number of packets to capture, defaults to None.

        Returns:
            subprocess: TShark process
        """
        if self._current_tshark:
            return self._current_tshark
        proc = await super(InMemCapture, self)._get_tshark_process(
            packet_count=packet_count, stdin=subprocess.PIPE
        )
    ...
    def _get_json_separators(self):
        ...
        if self._get_tshark_version() >= version.parse("2.6.7"):
            ...
            return (
                f"}}{os.linesep}{os.linesep}".encode(),
                f"}}{os.linesep}{os.linesep}]",
                1,
            )

    def _write_packet(self, packet, sniff_time):
        """Writes a packet to the TShark process STDIN for parsing.

        Args:
            packet (bytes): Packet data being written.
            sniff_time (float | datetime.datetime | None): Packet's sniff time - current time if None.
        """
    ...
    async def _get_parsed_packet_from_tshark(self, callback, timeout):
        ...
        try:
            ...
            raise asyncio.TimeoutError(
                "Timed out while waiting for tshark to parse packet. "
