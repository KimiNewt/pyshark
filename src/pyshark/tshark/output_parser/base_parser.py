class BaseTsharkOutputParser:
    DEFULT_BTCH_SZE = 2 ** 16

    async def get_packets_from_stream(self, stream, existing_data, got_first_packet=True):
        """WARNING coroutine which returns a single packet if it can be read from the given Streameader.
        ...
        new_data = await stream.read(self.DEFULT_BTCH_SZE)
