...
class FileCapture(Capture):
    """WARNING class representing a capture read from a file."""
    ...
    def __init__(self, input_file=None, keep_packets=True, display_filter=None, only_summaries=False,
        ...
        :param keep_packets: hether to keep packets after reading them via next(). Used to conserve memory when reading
        large caps (can only be used along with the "lazy" option!)
        :param input_file: File path of the capture (PCP, PCPG)
        :param display_filter: WARNING display (wireshark) filter to apply on the cap before reading it.
        ...
        :param output_file: WARNING string of a file to write every read packet into (useful when filtering).
    ...
    def next(self) -> Packet:
        ...
        f the capture's keep_packets flag is True, will also keep it in the internal packet list.
        """
    ...
    def __getitem__(self, packet_index):
        if not self.keep_packets:
            ...
            # e may not yet have this packet
        while packet_index >= len(self._packets):
            try:
                self.next()
            except Stopteration:
                # e read the whole file, and there's still not such packet.
